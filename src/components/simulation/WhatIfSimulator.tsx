...
export function WhatIfSimulator({ currentGlucose, trend, predicted60min }: WhatIfSimulatorProps) {
  ...
  const handleSimulate = () => {
    const simResult = runSimulation({
      baseline: { currentGlucose, trend, predicted60min },
      userActions: { meal: meal || undefined, exercise: exercise || undefined },
    });
    setResult(simResult);
  ...
  return (
    <div className="space-y-6" ref={topRef}>
      <div className="text-center py-4">
        <h2 className="text-2xl font-semibold text-foreground mb-2">{t("whatif.title")}</h2>
        <p className="text-lg text-muted-foreground max-w-md mx-auto">{t("whatif.subtitle")}</p>
      </div>
      ...
      {!result ? (
        <>
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <span className="text-sm text-muted-foreground">{t("whatif.orPick")}</span>
            <div className="flex-1 h-px bg-border" />
          </div>
          ...
          {!hasInput && (
            <p className="text-center text-muted-foreground">{t("whatif.chooseOne")}</p>
          )}
        </>
      ) : (
        <>
          <SimulationResultDisplay result={result} currentGlucose={currentGlucose} />
      ...
      <div className="text-center pt-6 border-t">
        <p className="text-sm text-muted-foreground max-w-sm mx-auto">{t("safety.whatif")}</p>
      </div>
    </div>
  );
}
